...
(defun vector-direction (v)
  ...
  (defun vector-rotate (v b-angle)
    (let* ((p1 (nth 0 v))
           ...
           (p2x (x-of p2))
           (p2y (y-of p2))
           (angle (/ (- p2y p1y) (- p2x p1x)))
           (c-angle (+ angle b-angle))
           )
      (list (cos c-angle) (sin c-angle))
      ))


  (defun vector-direction-rotate (v b-angle)
    (let* ((p1x 0.0d0)
           (p1y 0.0d0)
           (p2x (x-of v))
           (p2y (y-of v))
...
#+BEGIN_SRC lisp :results output raw
  ...
  (setq length-vector1 (vector-length vector1))
  (setq direction-vector1 (vector-length vector1))
  ...
  (defun divide-vector (v step-length)
    (let* ((vdir (vector-direction v))
           (vdir-per (vector-direction-rotate vdir (/ *pi* 2.0d0)))
           (start-point (nth 0 v))
           (end-point (nth 1 v))
  ...
  (loop for i in (divide-vector vector4 l-step)
        do (format t "| ~8,3F |  ~8,3F |   ~%" (nth 0 i) (nth 1 i))
        )
#+END_SRC
...
** test-trochoidal-orthogonal-internal-496

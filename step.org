...
#+BEGIN_SRC lisp :noweb yes :tangle step.lisp :comments both
    (load #P"/home/quill/quicklisp/setup.lisp")
...
#+BEGIN_SRC lisp :results output raw
  ...
  (setq a-angle (* *pi* 0.5d0))
    (let ((vdir (normal-vector v)))
      ...
      (format t "|direction-angle:| ~8,4F |  |~%" (rad-to-deg(normal-vector-angle v)))
      ...
      (let ((v-rotate (vector-rotate v a-angle)))
        ...
        (format t "|direction-angle:| ~8,4F |  |~%" (rad-to-deg(normal-vector-angle v-rotate)))
        ))
...
| direction:        |        0.0000 |       -1.0000 |
| direction-rotate: | (0.0d0 0.0d0) | (1.0d0 0.0d0) |
...
#+BEGIN_SRC lisp :results output raw
  ...
  (setq a-angle (* *pi* 0.5d0))
    (let ((vdir (normal-vector v)))
      (format t "|point1:| ~8,4F | ~8,4F |~%" (x-of point3) (y-of point3))
      (format t "|point2:| ~8,4F | ~8,4F |~%" (x-of point4) (y-of point4))
      (format t "|direction-angle:| ~8,4F |  |~%" (normal-vector-angle v))
...
#+BEGIN_SRC lisp :results output raw :tangle yes :noweb yes
    ...
    (setq new-start-point (p+ start-point (scale-vector vdir-perpendicular offset)))
...
#+BEGIN_SRC lisp :tangle yes :results output file :file trochoidal-circle-610--3.0mm-diameter-external.ngc
  (setq output-stream *STANDARD-OUTPUT*)
                    ;;;linear step
  (setq l-step 10.0d0)
  (setq zi 0.0)
